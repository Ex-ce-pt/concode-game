...
(function (GameState) {
    ...
    GameState[GameState["PATH_VIEW"] = 1] = "PATH_VIEW";
    GameState[GameState["GAME"] = 2] = "GAME";
    GameState[GameState["WON"] = 3] = "WON";
    GameState[GameState["LOST"] = 4] = "LOST";
})(GameState || (GameState = {}));
const GRID_WIDTH = 5;
const GRID_HEIGHT = 5;
...
const canvas = document.querySelector('canvas.game-field');
const ctx = canvas.getContext('2d');
const timerFiller = document.querySelector('span.timer-filler');
let player = { x: 0, y: 0 };
let finishTile = { x: 0, y: 0 };
let currentPath;
...
const tick = () => {
    ...
    if (deltaTime >= 1000 && gameState === GameState.PATH_VIEW) {
    ...
    else if (deltaTime >= 1000 && gameState === GameState.WON) {
        if (numWins === 3)
            gameTime = 950;
        else if (numWins === 5)
            gameTime = 900;
